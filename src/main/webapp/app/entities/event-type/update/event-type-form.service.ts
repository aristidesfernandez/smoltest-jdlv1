...
type EventTypeFormDefaults = Pick<NewEventType, 'id' | 'isStorable' | 'isPriority' | 'isAlarm' | 'idEventTypes'>;
...
type EventTypeFormGroupContent = {
  ...
  idEventTypes: FormControl<IEventType['idEventTypes']>;
};
...
export class EventTypeFormService {
  createEventTypeFormGroup(eventType: EventTypeFormGroupInput = { id: null }): EventTypeFormGroup {
    ...
    return new FormGroup<EventTypeFormGroupContent>({
      ...
      idEventTypes: new FormControl(eventTypeRawValue.idEventTypes ?? []),
    });
  }
