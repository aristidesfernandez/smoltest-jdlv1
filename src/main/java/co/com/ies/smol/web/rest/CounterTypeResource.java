...
public class CounterTypeResource {
    ...
    public ResponseEntity<CounterTypeDTO> createCounterType(@Valid @RequestBody CounterTypeDTO counterTypeDTO) throws URISyntaxException {
        ...
        return ResponseEntity
            .created(new URI("/api/counter-types/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, result.getId().toString()))
            .body(result);
    }

    /**
     * {@code PUT  /counter-types/:id} : Updates an existing counterType.
     *
     * @param id the id of the counterTypeDTO to save.
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/counter-types/{id}")
    public ResponseEntity<CounterTypeDTO> updateCounterType(
        @PathVariable(value = "id", required = false) final Long id,
        @Valid @RequestBody CounterTypeDTO counterTypeDTO
    ) throws URISyntaxException {
        log.debug("REST request to update CounterType : {}, {}", id, counterTypeDTO);
        if (counterTypeDTO.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, counterTypeDTO.getId())) {
            throw new BadRequestAlertException("Invalid ID", ENTITY_NAME, "idinvalid");
        }

        if (!counterTypeRepository.existsById(id)) {
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, counterTypeDTO.getId().toString()))
            .body(result);
    }

    /**
     * {@code PATCH  /counter-types/:id} : Partial updates given fields of an existing counterType, field will ignore if it is null
     *
     * @param id the id of the counterTypeDTO to save.
     * @param counterTypeDTO the counterTypeDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated counterTypeDTO,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/counter-types/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<CounterTypeDTO> partialUpdateCounterType(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody CounterTypeDTO counterTypeDTO
    ) throws URISyntaxException {
        log.debug("REST request to partial update CounterType partially : {}, {}", id, counterTypeDTO);
        if (counterTypeDTO.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, counterTypeDTO.getId())) {
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, counterTypeDTO.getId().toString())
        );
    }
    ...
    public ResponseEntity<CounterTypeDTO> getCounterType(@PathVariable Long id) {
    ...
    public ResponseEntity<Void> deleteCounterType(@PathVariable Long id) {
