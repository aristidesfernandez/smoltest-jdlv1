...
class EventTypeResourceIT {
    ...
    @Mock
    private EventTypeService eventTypeServiceMock;
    ...
    void getAllEventTypesWithEagerRelationshipsIsEnabled() throws Exception {
        when(eventTypeServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restEventTypeMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(eventTypeServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllEventTypesWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(eventTypeServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restEventTypeMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(eventTypeRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
    ...
    @Test
    @Transactional
    void getAllEventTypesByIdEventTypeIsEqualToSomething() throws Exception {
        Model idEventType;
        if (TestUtil.findAll(em, Model.class).isEmpty()) {
            eventTypeRepository.saveAndFlush(eventType);
            idEventType = ModelResourceIT.createEntity(em);
        } else {
            idEventType = TestUtil.findAll(em, Model.class).get(0);
        }
        em.persist(idEventType);
        em.flush();
        eventType.addIdEventType(idEventType);
        eventTypeRepository.saveAndFlush(eventType);
        Long idEventTypeId = idEventType.getId();

        // Get all the eventTypeList where idEventType equals to idEventTypeId
        defaultEventTypeShouldBeFound("idEventTypeId.equals=" + idEventTypeId);

        // Get all the eventTypeList where idEventType equals to (idEventTypeId + 1)
        defaultEventTypeShouldNotBeFound("idEventTypeId.equals=" + (idEventTypeId + 1));
    }
