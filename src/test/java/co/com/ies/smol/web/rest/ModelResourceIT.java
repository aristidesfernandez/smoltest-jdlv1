...
class ModelResourceIT {
    ...
    public static Model createEntity(EntityManager em) {
        ...
        model.setIdManufacturer(manufacturer);
        // Add required entity
        Formula formula;
    ...
    public static Model createUpdatedEntity(EntityManager em) {
        ...
        Formula formula;
        if (TestUtil.findAll(em, Formula.class).isEmpty()) {
            formula = FormulaResourceIT.createUpdatedEntity(em);
            em.persist(formula);
            em.flush();
        } else {
            formula = TestUtil.findAll(em, Formula.class).get(0);
    ...
    @Test
    @Transactional
    void getAllModelsByIdManufacturerIsEqualToSomething() throws Exception {
        Manufacturer idManufacturer;
        if (TestUtil.findAll(em, Manufacturer.class).isEmpty()) {
            modelRepository.saveAndFlush(model);
            idManufacturer = ManufacturerResourceIT.createEntity(em);
        } else {
            idManufacturer = TestUtil.findAll(em, Manufacturer.class).get(0);
        }
        ...
        // Get all the modelList where idManufacturer equals to idManufacturerId
        defaultModelShouldBeFound("idManufacturerId.equals=" + idManufacturerId);

        // Get all the modelList where idManufacturer equals to (idManufacturerId + 1)
        defaultModelShouldNotBeFound("idManufacturerId.equals=" + (idManufacturerId + 1));
    }

    @Test
    @Transactional
    void getAllModelsByIdFormulaIsEqualToSomething() throws Exception {
        Formula idFormula;
        if (TestUtil.findAll(em, Formula.class).isEmpty()) {
            modelRepository.saveAndFlush(model);
            idFormula = FormulaResourceIT.createEntity(em);
        } else {
            idFormula = TestUtil.findAll(em, Formula.class).get(0);
        }
        em.persist(idFormula);
        em.flush();
        model.setIdFormula(idFormula);
        modelRepository.saveAndFlush(model);
        Long idFormulaId = idFormula.getId();

        // Get all the modelList where idFormula equals to idFormulaId
        defaultModelShouldBeFound("idFormulaId.equals=" + idFormulaId);

        // Get all the modelList where idFormula equals to (idFormulaId + 1)
        defaultModelShouldNotBeFound("idFormulaId.equals=" + (idFormulaId + 1));
    }

    @Test
    @Transactional
    void getAllModelsByIdModelIsEqualToSomething() throws Exception {
        EventType idModel;
        if (TestUtil.findAll(em, EventType.class).isEmpty()) {
            modelRepository.saveAndFlush(model);
            idModel = EventTypeResourceIT.createEntity(em);
        } else {
            idModel = TestUtil.findAll(em, EventType.class).get(0);
        }
        em.persist(idModel);
        em.flush();
        model.addIdModel(idModel);
        modelRepository.saveAndFlush(model);
        Long idModelId = idModel.getId();

        // Get all the modelList where idModel equals to idModelId
        defaultModelShouldBeFound("idModelId.equals=" + idModelId);

        // Get all the modelList where idModel equals to (idModelId + 1)
        defaultModelShouldNotBeFound("idModelId.equals=" + (idModelId + 1));
    }
