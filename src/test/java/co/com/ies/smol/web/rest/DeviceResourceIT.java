...
class DeviceResourceIT {
    ...
    public static Device createEntity(EntityManager em) {
        ...
        if (TestUtil.findAll(em, Interfacing.class).isEmpty()) {
            interfacing = InterfacingResourceIT.createEntity(em);
            em.persist(interfacing);
            em.flush();
        } else {
            interfacing = TestUtil.findAll(em, Interfacing.class).get(0);
        }
        device.setIdInterfacing(interfacing);
        // Add required entity
        Model model;
        ...
        device.setIdDeviceCategory(deviceCategory);
        // Add required entity
        ...
        device.setIdDeviceType(deviceType);
        return device;
    }
    ...
    public static Device createUpdatedEntity(EntityManager em) {
        ...
        Interfacing interfacing;
        if (TestUtil.findAll(em, Interfacing.class).isEmpty()) {
            interfacing = InterfacingResourceIT.createUpdatedEntity(em);
            em.persist(interfacing);
            em.flush();
        } else {
            interfacing = TestUtil.findAll(em, Interfacing.class).get(0);
    ...
    @Test
    @Transactional
    void getAllDevicesByIdInterfacingIsEqualToSomething() throws Exception {
        Interfacing idInterfacing;
        if (TestUtil.findAll(em, Interfacing.class).isEmpty()) {
            deviceRepository.saveAndFlush(device);
            idInterfacing = InterfacingResourceIT.createEntity(em);
        } else {
            idInterfacing = TestUtil.findAll(em, Interfacing.class).get(0);
        }
        em.persist(idInterfacing);
        em.flush();
        device.setIdInterfacing(idInterfacing);
        deviceRepository.saveAndFlush(device);
        Long idInterfacingId = idInterfacing.getId();

        // Get all the deviceList where idInterfacing equals to idInterfacingId
        defaultDeviceShouldBeFound("idInterfacingId.equals=" + idInterfacingId);

        // Get all the deviceList where idInterfacing equals to (idInterfacingId + 1)
        defaultDeviceShouldNotBeFound("idInterfacingId.equals=" + (idInterfacingId + 1));
    }

    @Test
    @Transactional
    void getAllDevicesByIdModelIsEqualToSomething() throws Exception {
        Model idModel;
        if (TestUtil.findAll(em, Model.class).isEmpty()) {
            deviceRepository.saveAndFlush(device);
            idModel = ModelResourceIT.createEntity(em);
        } else {
            idModel = TestUtil.findAll(em, Model.class).get(0);
        }
        em.persist(idModel);
        em.flush();
        device.setIdModel(idModel);
        deviceRepository.saveAndFlush(device);
        Long idModelId = idModel.getId();

        // Get all the deviceList where idModel equals to idModelId
        defaultDeviceShouldBeFound("idModelId.equals=" + idModelId);

        // Get all the deviceList where idModel equals to (idModelId + 1)
        defaultDeviceShouldNotBeFound("idModelId.equals=" + (idModelId + 1));
    }

    @Test
    @Transactional
    void getAllDevicesByIdDeviceCategoryIsEqualToSomething() throws Exception {
        DeviceCategory idDeviceCategory;
        if (TestUtil.findAll(em, DeviceCategory.class).isEmpty()) {
            deviceRepository.saveAndFlush(device);
            idDeviceCategory = DeviceCategoryResourceIT.createEntity(em);
        } else {
            idDeviceCategory = TestUtil.findAll(em, DeviceCategory.class).get(0);
        }
        em.persist(idDeviceCategory);
        em.flush();
        device.setIdDeviceCategory(idDeviceCategory);
        deviceRepository.saveAndFlush(device);
        Long idDeviceCategoryId = idDeviceCategory.getId();

        // Get all the deviceList where idDeviceCategory equals to idDeviceCategoryId
        defaultDeviceShouldBeFound("idDeviceCategoryId.equals=" + idDeviceCategoryId);

        // Get all the deviceList where idDeviceCategory equals to (idDeviceCategoryId + 1)
        defaultDeviceShouldNotBeFound("idDeviceCategoryId.equals=" + (idDeviceCategoryId + 1));
    }

    @Test
    @Transactional
    void getAllDevicesByIdDeviceTypeIsEqualToSomething() throws Exception {
        DeviceType idDeviceType;
        if (TestUtil.findAll(em, DeviceType.class).isEmpty()) {
            deviceRepository.saveAndFlush(device);
            idDeviceType = DeviceTypeResourceIT.createEntity(em);
        } else {
            idDeviceType = TestUtil.findAll(em, DeviceType.class).get(0);
        }
        em.persist(idDeviceType);
        em.flush();
        device.setIdDeviceType(idDeviceType);
        deviceRepository.saveAndFlush(device);
        Long idDeviceTypeId = idDeviceType.getId();

        // Get all the deviceList where idDeviceType equals to idDeviceTypeId
        defaultDeviceShouldBeFound("idDeviceTypeId.equals=" + idDeviceTypeId);

        // Get all the deviceList where idDeviceType equals to (idDeviceTypeId + 1)
        defaultDeviceShouldNotBeFound("idDeviceTypeId.equals=" + (idDeviceTypeId + 1));
    }

    @Test
    @Transactional
    void getAllDevicesByIdFormulaHandpayIsEqualToSomething() throws Exception {
        Formula idFormulaHandpay;
        if (TestUtil.findAll(em, Formula.class).isEmpty()) {
            deviceRepository.saveAndFlush(device);
            idFormulaHandpay = FormulaResourceIT.createEntity(em);
        } else {
            idFormulaHandpay = TestUtil.findAll(em, Formula.class).get(0);
        }
        em.persist(idFormulaHandpay);
        em.flush();
        device.setIdFormulaHandpay(idFormulaHandpay);
        deviceRepository.saveAndFlush(device);
        Long idFormulaHandpayId = idFormulaHandpay.getId();

        // Get all the deviceList where idFormulaHandpay equals to idFormulaHandpayId
        defaultDeviceShouldBeFound("idFormulaHandpayId.equals=" + idFormulaHandpayId);

        // Get all the deviceList where idFormulaHandpay equals to (idFormulaHandpayId + 1)
        defaultDeviceShouldNotBeFound("idFormulaHandpayId.equals=" + (idFormulaHandpayId + 1));
    }

    @Test
    @Transactional
    void getAllDevicesByIdFormulaJackpotIsEqualToSomething() throws Exception {
        Formula idFormulaJackpot;
        if (TestUtil.findAll(em, Formula.class).isEmpty()) {
            deviceRepository.saveAndFlush(device);
            idFormulaJackpot = FormulaResourceIT.createEntity(em);
        } else {
            idFormulaJackpot = TestUtil.findAll(em, Formula.class).get(0);
        }
        em.persist(idFormulaJackpot);
        em.flush();
        device.setIdFormulaJackpot(idFormulaJackpot);
        deviceRepository.saveAndFlush(device);
        Long idFormulaJackpotId = idFormulaJackpot.getId();

        // Get all the deviceList where idFormulaJackpot equals to idFormulaJackpotId
        defaultDeviceShouldBeFound("idFormulaJackpotId.equals=" + idFormulaJackpotId);

        // Get all the deviceList where idFormulaJackpot equals to (idFormulaJackpotId + 1)
        defaultDeviceShouldNotBeFound("idFormulaJackpotId.equals=" + (idFormulaJackpotId + 1));
    }

    @Test
    @Transactional
    void getAllDevicesByIdIslandIsEqualToSomething() throws Exception {
        Island idIsland;
        if (TestUtil.findAll(em, Island.class).isEmpty()) {
            deviceRepository.saveAndFlush(device);
            idIsland = IslandResourceIT.createEntity(em);
        } else {
            idIsland = TestUtil.findAll(em, Island.class).get(0);
        }
        em.persist(idIsland);
        em.flush();
        device.setIdIsland(idIsland);
        deviceRepository.saveAndFlush(device);
        Long idIslandId = idIsland.getId();

        // Get all the deviceList where idIsland equals to idIslandId
        defaultDeviceShouldBeFound("idIslandId.equals=" + idIslandId);

        // Get all the deviceList where idIsland equals to (idIslandId + 1)
        defaultDeviceShouldNotBeFound("idIslandId.equals=" + (idIslandId + 1));
    }

    @Test
    @Transactional
    void getAllDevicesByIdCurrencyTypeIsEqualToSomething() throws Exception {
        CurrencyType idCurrencyType;
        if (TestUtil.findAll(em, CurrencyType.class).isEmpty()) {
            deviceRepository.saveAndFlush(device);
            idCurrencyType = CurrencyTypeResourceIT.createEntity(em);
        } else {
            idCurrencyType = TestUtil.findAll(em, CurrencyType.class).get(0);
        }
        em.persist(idCurrencyType);
        em.flush();
        device.setIdCurrencyType(idCurrencyType);
        deviceRepository.saveAndFlush(device);
        Long idCurrencyTypeId = idCurrencyType.getId();

        // Get all the deviceList where idCurrencyType equals to idCurrencyTypeId
        defaultDeviceShouldBeFound("idCurrencyTypeId.equals=" + idCurrencyTypeId);

        // Get all the deviceList where idCurrencyType equals to (idCurrencyTypeId + 1)
        defaultDeviceShouldNotBeFound("idCurrencyTypeId.equals=" + (idCurrencyTypeId + 1));
    }
