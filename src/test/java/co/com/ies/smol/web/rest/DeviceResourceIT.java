...
class DeviceResourceIT {
    ...
    public static Device createEntity(EntityManager em) {
        ...
        device.setInterfacing(interfacing);
        // Add required entity
        Model model;
    ...
    @Test
    @Transactional
    void getAllDevicesByInterfacingIsEqualToSomething() throws Exception {
        Interfacing interfacing;
        if (TestUtil.findAll(em, Interfacing.class).isEmpty()) {
            deviceRepository.saveAndFlush(device);
            interfacing = InterfacingResourceIT.createEntity(em);
        ...
        device.setInterfacing(interfacing);
        deviceRepository.saveAndFlush(device);
        Long interfacingId = interfacing.getId();

        // Get all the deviceList where interfacing equals to interfacingId
        defaultDeviceShouldBeFound("interfacingId.equals=" + interfacingId);

        // Get all the deviceList where interfacing equals to (interfacingId + 1)
        defaultDeviceShouldNotBeFound("interfacingId.equals=" + (interfacingId + 1));
    }
    ...
    @Test
    @Transactional
    void getAllDevicesByIslandIsEqualToSomething() throws Exception {
        Island island;
        if (TestUtil.findAll(em, Island.class).isEmpty()) {
            deviceRepository.saveAndFlush(device);
            island = IslandResourceIT.createEntity(em);
        } else {
            island = TestUtil.findAll(em, Island.class).get(0);
        }
        em.persist(island);
        em.flush();
        device.setIsland(island);
        deviceRepository.saveAndFlush(device);
        Long islandId = island.getId();

        // Get all the deviceList where island equals to islandId
        defaultDeviceShouldBeFound("islandId.equals=" + islandId);

        // Get all the deviceList where island equals to (islandId + 1)
        defaultDeviceShouldNotBeFound("islandId.equals=" + (islandId + 1));
    }

    @Test
    @Transactional
    void getAllDevicesByUrrencyTypeIsEqualToSomething() throws Exception {
        CurrencyType urrencyType;
        if (TestUtil.findAll(em, CurrencyType.class).isEmpty()) {
            deviceRepository.saveAndFlush(device);
            urrencyType = CurrencyTypeResourceIT.createEntity(em);
        } else {
            urrencyType = TestUtil.findAll(em, CurrencyType.class).get(0);
        }
        em.persist(urrencyType);
        em.flush();
        device.setUrrencyType(urrencyType);
        deviceRepository.saveAndFlush(device);
        Long urrencyTypeId = urrencyType.getId();

        // Get all the deviceList where urrencyType equals to urrencyTypeId
        defaultDeviceShouldBeFound("urrencyTypeId.equals=" + urrencyTypeId);

        // Get all the deviceList where urrencyType equals to (urrencyTypeId + 1)
        defaultDeviceShouldNotBeFound("urrencyTypeId.equals=" + (urrencyTypeId + 1));
    }
