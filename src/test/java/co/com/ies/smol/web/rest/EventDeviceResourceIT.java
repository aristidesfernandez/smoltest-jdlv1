...
class EventDeviceResourceIT {
    ...
    public static EventDevice createEntity(EntityManager em) {
        ...
        eventDevice.setEstablishment(establishment);
        // Add required entity
        EventType eventType;
    ...
    @Test
    @Transactional
    void getAllEventDevicesByEstablishmentIsEqualToSomething() throws Exception {
        Establishment establishment;
        if (TestUtil.findAll(em, Establishment.class).isEmpty()) {
            eventDeviceRepository.saveAndFlush(eventDevice);
            establishment = EstablishmentResourceIT.createEntity(em);
        ...
        // Get all the eventDeviceList where establishment equals to establishmentId
        defaultEventDeviceShouldBeFound("establishmentId.equals=" + establishmentId);

        // Get all the eventDeviceList where establishment equals to (establishmentId + 1)
        defaultEventDeviceShouldNotBeFound("establishmentId.equals=" + (establishmentId + 1));
    }
