...
class CounterTypeResourceIT {
    ...
    private static final String DEFAULT_COUNTER_CODE = "AAAAAAAAAA";
    private static final String UPDATED_COUNTER_CODE = "BBBBBBBBBB";
    ...
    void createCounterType() throws Exception {
        ...
        assertThat(testCounterType.getCounterCode()).isEqualTo(DEFAULT_COUNTER_CODE);
    ...
    void checkCounterCodeIsRequired() throws Exception {
        int databaseSizeBeforeTest = counterTypeRepository.findAll().size();
        // set the field null
        counterType.setCounterCode(null);

        // Create the CounterType, which fails.
        CounterTypeDTO counterTypeDTO = counterTypeMapper.toDto(counterType);
        ...
        List<CounterType> counterTypeList = counterTypeRepository.findAll();
        assertThat(counterTypeList).hasSize(databaseSizeBeforeTest);
    }
    ...
    void getAllCounterTypes() throws Exception {
        ...
        restCounterTypeMockMvc
            ...
            .andExpect(jsonPath("$.[*].id").value(hasItem(counterType.getId().intValue())))
    ...
    void getCounterType() throws Exception {
        ...
        // Get the counterType
        restCounterTypeMockMvc
            .perform(get(ENTITY_API_URL_ID, counterType.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.id").value(counterType.getId().intValue()))
    ...
    void getCounterTypesByIdFiltering() throws Exception {
        ...
        Long id = counterType.getId();

        defaultCounterTypeShouldBeFound("id.equals=" + id);
        defaultCounterTypeShouldNotBeFound("id.notEquals=" + id);

        defaultCounterTypeShouldBeFound("id.greaterThanOrEqual=" + id);
        defaultCounterTypeShouldNotBeFound("id.greaterThan=" + id);

        defaultCounterTypeShouldBeFound("id.lessThanOrEqual=" + id);
        defaultCounterTypeShouldNotBeFound("id.lessThan=" + id);
    }

    @Test
    @Transactional
    void getAllCounterTypesByCounterCodeIsEqualToSomething() throws Exception {
        // Initialize the database
        counterTypeRepository.saveAndFlush(counterType);

        // Get all the counterTypeList where counterCode equals to DEFAULT_COUNTER_CODE
        defaultCounterTypeShouldBeFound("counterCode.equals=" + DEFAULT_COUNTER_CODE);

        // Get all the counterTypeList where counterCode equals to UPDATED_COUNTER_CODE
        defaultCounterTypeShouldNotBeFound("counterCode.equals=" + UPDATED_COUNTER_CODE);
    }

    @Test
    @Transactional
    void getAllCounterTypesByCounterCodeIsInShouldWork() throws Exception {
        // Initialize the database
        counterTypeRepository.saveAndFlush(counterType);

        // Get all the counterTypeList where counterCode in DEFAULT_COUNTER_CODE or UPDATED_COUNTER_CODE
        defaultCounterTypeShouldBeFound("counterCode.in=" + DEFAULT_COUNTER_CODE + "," + UPDATED_COUNTER_CODE);

        // Get all the counterTypeList where counterCode equals to UPDATED_COUNTER_CODE
        defaultCounterTypeShouldNotBeFound("counterCode.in=" + UPDATED_COUNTER_CODE);
    }

    @Test
    @Transactional
    void getAllCounterTypesByCounterCodeIsNullOrNotNull() throws Exception {
        // Initialize the database
        counterTypeRepository.saveAndFlush(counterType);

        // Get all the counterTypeList where counterCode is not null
        defaultCounterTypeShouldBeFound("counterCode.specified=true");

        // Get all the counterTypeList where counterCode is null
        defaultCounterTypeShouldNotBeFound("counterCode.specified=false");
    }

    @Test
    @Transactional
    void getAllCounterTypesByCounterCodeContainsSomething() throws Exception {
        // Initialize the database
        counterTypeRepository.saveAndFlush(counterType);

        // Get all the counterTypeList where counterCode contains DEFAULT_COUNTER_CODE
        defaultCounterTypeShouldBeFound("counterCode.contains=" + DEFAULT_COUNTER_CODE);

        // Get all the counterTypeList where counterCode contains UPDATED_COUNTER_CODE
        defaultCounterTypeShouldNotBeFound("counterCode.contains=" + UPDATED_COUNTER_CODE);
    }

    @Test
    @Transactional
    void getAllCounterTypesByCounterCodeNotContainsSomething() throws Exception {
        // Initialize the database
        counterTypeRepository.saveAndFlush(counterType);

        // Get all the counterTypeList where counterCode does not contain DEFAULT_COUNTER_CODE
        defaultCounterTypeShouldNotBeFound("counterCode.doesNotContain=" + DEFAULT_COUNTER_CODE);

        // Get all the counterTypeList where counterCode does not contain UPDATED_COUNTER_CODE
        defaultCounterTypeShouldBeFound("counterCode.doesNotContain=" + UPDATED_COUNTER_CODE);
    }
    ...
    void putExistingCounterType() throws Exception {
        ...
        CounterType updatedCounterType = counterTypeRepository.findById(counterType.getId()).get();
        ...
        restCounterTypeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, counterTypeDTO.getId())
        ...
        CounterType testCounterType = counterTypeList.get(counterTypeList.size() - 1);
        assertThat(testCounterType.getCounterCode()).isEqualTo(UPDATED_COUNTER_CODE);
    ...
    void putNonExistingCounterType() throws Exception {
        int databaseSizeBeforeUpdate = counterTypeRepository.findAll().size();
        counterType.setId(count.incrementAndGet());
    ...
    void partialUpdateCounterTypeWithPatch() throws Exception {
        ...
        partialUpdatedCounterType.setId(counterType.getId());

        partialUpdatedCounterType.name(UPDATED_NAME).description(UPDATED_DESCRIPTION).prize(UPDATED_PRIZE);

        restCounterTypeMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, partialUpdatedCounterType.getId())
    ...
    void patchNonExistingCounterType() throws Exception {
        ...
        restCounterTypeMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, counterTypeDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(counterTypeDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void deleteCounterType() throws Exception {
        ...
        restCounterTypeMockMvc
            .perform(delete(ENTITY_API_URL_ID, counterType.getId()).accept(MediaType.APPLICATION_JSON))
