...
class CounterDeviceResourceIT {
    ...
    public static CounterDevice createEntity(EntityManager em) {
        ...
        counterDevice.setCounter(counterType);
        // Add required entity
        Device device;
    ...
    @Test
    @Transactional
    void getAllCounterDevicesByCounterIsEqualToSomething() throws Exception {
        CounterType counter;
        if (TestUtil.findAll(em, CounterType.class).isEmpty()) {
            counterDeviceRepository.saveAndFlush(counterDevice);
            counter = CounterTypeResourceIT.createEntity(em);
        } else {
            counter = TestUtil.findAll(em, CounterType.class).get(0);
        }
        em.persist(counter);
        em.flush();
        counterDevice.setCounter(counter);
        counterDeviceRepository.saveAndFlush(counterDevice);
        String counterId = counter.getCounterCode();

        // Get all the counterDeviceList where counter equals to counterId
        defaultCounterDeviceShouldBeFound("counterId.equals=" + counterId);

        // Get all the counterDeviceList where counter equals to "invalid-id"
        defaultCounterDeviceShouldNotBeFound("counterId.equals=" + "invalid-id");
    }
