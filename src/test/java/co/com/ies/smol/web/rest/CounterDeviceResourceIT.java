...
class CounterDeviceResourceIT {
    ...
    public static CounterDevice createEntity(EntityManager em) {
        ...
        counterDevice.setCounterCode(counterType);
        // Add required entity
        Device device;
        ...
        counterDevice.setIdDevice(device);
    ...
    void getAllCounterDevices() throws Exception {
        ...
        restCounterDeviceMockMvc
            ...
            .andExpect(jsonPath("$.[*].id").value(hasItem(counterDevice.getId().intValue())))
    ...
    void getCounterDevice() throws Exception {
        ...
        restCounterDeviceMockMvc
            ...
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.id").value(counterDevice.getId().intValue()))
    ...
    void getCounterDevicesByIdFiltering() throws Exception {
        ...
        Long id = counterDevice.getId();

        defaultCounterDeviceShouldBeFound("id.equals=" + id);
        defaultCounterDeviceShouldNotBeFound("id.notEquals=" + id);

        defaultCounterDeviceShouldBeFound("id.greaterThanOrEqual=" + id);
        defaultCounterDeviceShouldNotBeFound("id.greaterThan=" + id);

        defaultCounterDeviceShouldBeFound("id.lessThanOrEqual=" + id);
        defaultCounterDeviceShouldNotBeFound("id.lessThan=" + id);
    }
    ...
    @Test
    @Transactional
    void getAllCounterDevicesByCounterCodeIsEqualToSomething() throws Exception {
        ...
        em.flush();
        counterDevice.setCounterCode(counterCode);
        counterDeviceRepository.saveAndFlush(counterDevice);
        Long counterCodeId = counterCode.getId();

        // Get all the counterDeviceList where counterCode equals to counterCodeId
        defaultCounterDeviceShouldBeFound("counterCodeId.equals=" + counterCodeId);

        // Get all the counterDeviceList where counterCode equals to (counterCodeId + 1)
        defaultCounterDeviceShouldNotBeFound("counterCodeId.equals=" + (counterCodeId + 1));
    }

    @Test
    @Transactional
    void getAllCounterDevicesByIdDeviceIsEqualToSomething() throws Exception {
        Device idDevice;
        if (TestUtil.findAll(em, Device.class).isEmpty()) {
            counterDeviceRepository.saveAndFlush(counterDevice);
            idDevice = DeviceResourceIT.createEntity(em);
        } else {
            idDevice = TestUtil.findAll(em, Device.class).get(0);
        }
        em.persist(idDevice);
        em.flush();
        counterDevice.setIdDevice(idDevice);
        counterDeviceRepository.saveAndFlush(counterDevice);
        Long idDeviceId = idDevice.getId();

        // Get all the counterDeviceList where idDevice equals to idDeviceId
        defaultCounterDeviceShouldBeFound("idDeviceId.equals=" + idDeviceId);

        // Get all the counterDeviceList where idDevice equals to (idDeviceId + 1)
        defaultCounterDeviceShouldNotBeFound("idDeviceId.equals=" + (idDeviceId + 1));
    }
    ...
    void getNonExistingCounterDevice() throws Exception {
        // Get the counterDevice
        restCounterDeviceMockMvc.perform(get(ENTITY_API_URL_ID, Long.MAX_VALUE)).andExpect(status().isNotFound());
    }
    ...
    void putNonExistingCounterDevice() throws Exception {
        int databaseSizeBeforeUpdate = counterDeviceRepository.findAll().size();
        counterDevice.setId(count.incrementAndGet());
    ...
    void deleteCounterDevice() throws Exception {
        ...
        restCounterDeviceMockMvc
            .perform(delete(ENTITY_API_URL_ID, counterDevice.getId()).accept(MediaType.APPLICATION_JSON))
